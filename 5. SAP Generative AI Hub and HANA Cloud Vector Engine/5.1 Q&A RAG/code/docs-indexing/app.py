...
if __name__ == "__main__":
    ...
    try:
        ...
        document_to_index = [
            [idx + 1, doc.metadata.get('title', ''), doc.metadata.get('page_number', ''), doc.metadata.get('document_url', ''), doc.page_content, str(get_embedding(doc.page_content))]
            for idx, doc in enumerate(split_docs) if get_embedding(doc.page_content)
        ]
